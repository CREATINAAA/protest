...
class SalesrenderFacade:
    ...
    @staticmethod
    def get_body_for_updating_status_cs(order_id, status_cs):
            ...
            updateOrder(
              input: {{
                ...
                orderData: {{
                  stringFields:
                    [
                      {{field: "status_cs", value: "{status_cs}" }},
                    ],
                  dateTimeFields: [
    ...
    def update_status_cs(self, order_id, status_cs):
        body = f""" 
        mutation {{
            {self.get_body_for_updating_status_cs(order_id, status_cs)}
        }}
        """
        return self._send_query(body)
    ...
    def update_several_statuses_cs(self, orders: dict):
        ...
        for i, (order_number, status) in enumerate(orders.items()):
            order_requests.append(
                f"orderMutation{i}: {self.get_body_for_updating_status_cs(self.get_order_id_by_order_number(order_number), status)}"
            )
        body = """ 
            mutation {{
                {orders}
